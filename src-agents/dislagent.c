...
// ***** reagent *****
 	tagger_newclass(jni, jvmti, loader, class_name, class_byte_count, class_bytes);
 }
  ...
  // Local initialization.
	...
	// register callbacks
 	jvmtiEventCallbacks callbacks = {
 		...
 		.ThreadEnd = &jvmti_callback_thread_end_hook,
 		.ClassPrepare = &jvmti_callback_class_prepare_hook,
 		.ObjectFree = &jvmti_callback_object_free_hook,
 	};
	...
	// ***** reagent *****
 	...
 	check_jvmti_error(jvmti, error, "Cannot set class prepare hook");

 	error = (*jvmti)->SetEventNotificationMode(jvmti, JVMTI_ENABLE, JVMTI_EVENT_OBJECT_FREE, NULL);
 	check_jvmti_error(jvmti, error, "Cannot set object free hook");

 	error = (*jvmti)->SetEventNotificationMode(jvmti, JVMTI_ENABLE, JVMTI_EVENT_THREAD_END, NULL);
 	check_jvmti_error(jvmti, error, "Cannot set thread end hook");

	// init blocking queues
 	redispatcher_init(jvmti);
 	glbuffer_init(jvmti);
 	tl_init(jvmti);
